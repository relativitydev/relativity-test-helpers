...
namespace Relativity.Test.Helpers.UserHelpers
{
	public class UserHelper
	{
		...
		{
			int clientID = ClientHelper.GetClientId(servicesMgr, Constants.TestHelperTemporaryValues.UserHelperTemporaryValues.Client).Result;
			int userTypeID = ChoiceHelper.GetChoiceId(servicesMgr, Constants.TestHelperTemporaryValues.UserHelperTemporaryValues.UserType).Result;
			int userId;
			...
			using (IUserInfoManager userManager = servicesMgr.CreateProxy<IUserInfoManager>(ExecutionIdentity.CurrentUser))
			{
				try
				{
					UserResponse userResponse = userManager.CreateAsync(request).Result;
		...
		{
			int clientID = ClientHelper.GetClientId(servicesMgr, clientName).Result;
			int userTypeID = ChoiceHelper.GetChoiceId(servicesMgr, userType).Result;
			int userId;
